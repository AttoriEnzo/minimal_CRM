...
$action = $_GET['action'] ?? '';
$id = $_GET['id'] ?? null;
$ajax_actions = ['update_status'];

// AGGIUNGI QUESTO BLOCCO PER IL DEBUG NEL TUO public/index.php
//if ($page === 'repairs' && $action === 'update_status') {
//    die('DEBUG ROUTER: index.php ha riconosciuto la rotta, il problema è nella logica che segue.');
//}
...
// ---> MODIFICA 2: Sostituisci il blocco sopra con questo <---
$special_pages = ['login', 'print_commercial_doc', 'print_technical_doc'];
if (!in_array($page, $special_pages) && !in_array($action, $ajax_actions)) {
...
// ---> MODIFICA 3: Sostituisci il blocco sopra con questo <---
if (!in_array($page, $special_pages) && !in_array($action, $ajax_actions)) {
